node {
	...
	stage('Sonar Scanner') {
      withSonarQubeEnv('SonarQube') {
	  	if (isUnix()) {
			sh "${scannerHome}/bin/sonar-scanner -Dsonar.projectKey=e2e-application -Dsonar.sources=."
		  } else {
	        bat(/${scannerHome}\bin\sonar-scanner -Dsonar.projectKey=e2e-application -Dsonar.sources=./)
		}
		/*
		dir("angular-app") {
		  if (isUnix()) {
